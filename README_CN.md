...
- **O(1) 分配/释放** —— 分配时把请求向上取整到块足够大的最小分级，从该级空闲链表弹出；
  释放在恒定时间内归还块。
- **每块零开销** —— 释放时仅凭地址区间就能把指针映射回所属分级；块本身不带任何头部。
- **无碎片** —— 同一分级内每个块都一样大，因此该池永不产生碎片。
- **可配置的向上借块** —— 理想分级用尽时，池可自动回退到更大的分级；在初始化时设置
  `forbid_fallback` 可改为严格限定只用最合适的分级。
- **单结构体配置** —— 所有初始化参数（缓冲、分级列表、策略）都放在一个
  `nx_tiered_mem_pool_cfg_t` 中，分级列表内联其中；初始化还会报告分级实际需要的确切
  字节数，因此你可以把缓冲开大一点，跑一次后再缩到刚好合适。
- **内置统计** —— 每级的块大小、块数量、空闲数量，以及用于调优和诊断的峰值占用
  （high-water mark）。
- **非线程安全** —— 并发访问必须由调用者加锁。
