...
一个仅头文件的事件标志模块：32 个位，可设置、清除、测试、原子地获取。每个位代表一个事件。
用于 ISR 与主循环之间、或需要广播就绪状态 / 协调关机的模块之间的轻量级信号传递。非阻塞：
在你运行时轮询这些标志，因此它们自然适配协作式循环，无需调度器。

- **32 个独立位** —— 每个位是一个事件（意义由用户定义）。
- **设置 / 清除 / 测试 / 获取** —— `set` 置位标志，`clear` 清零标志，`test` 检查（非消耗性），
  `take` 原子地测试并清零。对广播事件（每个模块都能看到）用 `test`，对一次性工作项（一个
  消费者获取）用 `take`。
- **屏障原语：test_all** —— 仅当掩码中的每个位都置位时才返回 true。用于确认屏障（等待三个模块
  各自设置自己的确认位）或多部分就绪检查。
- **合并** —— 在一次 take 之前多次设置同一标志，仍然只读出一个置位的位。标志告诉你
  "至少发生了一次 X"，而不是计数。
- **可选 ISR 安全** —— 在 init 时传入 `nx_lock` 使 set/clear/take 对中断抢占原子化。只读操作
  （`test`、`test_all`、`get`）无需锁（单个 32 位读取）。当所有访问都来自一个上下文时可省略锁。
- **零分配、仅头文件** —— 实例占 8 字节（一个 `uint32_t` 加一个锁指针）；所有操作都是
  `static inline`。
...
> **注意：** `test` / `test_all` 是只读的，保持标志置位 —— 对多个模块都需要看到的广播事件
> 使用这些。`take` 是原子的测试并清零（一个消费者获取，标志被清除）。在一次 `take` 之前的
> 多次 `set` 调用会合并为一个置位的位 —— 标志表示"至少一次"，不是计数。仅当 ISR 和主循环
> 共享实例时才在 init 时传入锁；单上下文使用无需锁。
