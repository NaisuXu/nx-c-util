...
uint16_t nx_modbus_rtu_crc16(const uint8_t *data, size_t len)
{
    ...
    if (data == NULL) {
        return crc;   /* nothing to fold in; init value */
    }
...
{
    if (frame == NULL || len < 5u) {   /* 5 = smallest valid ADU (exception rsp) */
        return false;
    }

    uint16_t crc = nx_modbus_rtu_crc16(frame, len - 2u);
    frame[len - 2u] = (uint8_t)(crc & 0xFFu);        /* crc_l first on the wire */
    frame[len - 1u] = (uint8_t)((crc >> 8) & 0xFFu); /* crc_h */
    return true;
}
...
bool nx_modbus_rtu_check_crc(const uint8_t *frame, size_t len)
{
    ...
    uint16_t computed = nx_modbus_rtu_crc16(frame, len - 2u);
    uint16_t stored   = (uint16_t)((uint16_t)frame[len - 2u] |
